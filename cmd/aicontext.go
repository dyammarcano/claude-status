...
var aicontextCmd = &cobra.Command{
	...
	Long: "Generate structured documentation about claude-status for use by AI tools.\n" +
		"\n" +
		"Outputs a markdown (or JSON) reference of all commands, flags, and usage\n" +
		"patterns that AI assistants can consume as context.\n" +
		"\n" +
		"Examples:\n" +
		"  claude-status aicontext             # Markdown output (default)\n" +
		"  claude-status aicontext --json      # Structured JSON\n" +
		"  claude-status aicontext --compact   # Shorter output",
	RunE: runAIContext,
}
...
func printAIContextMarkdown(cmd *cobra.Command) error {
	...
	b.WriteString("# claude-status - AI Context\n\n")
	b.WriteString("## Overview\n\n")
	b.WriteString("claude-status is a CLI application\n\n")
...
func printAIContextCompact(cmd *cobra.Command) error {
	...
	b.WriteString("# claude-status - claude-status is a CLI application\n\n")
...
func printAIContextJSON(cmd *cobra.Command) error {
	doc := aiContextDoc{
		Tool:        "claude-status",
		Version:     "dev",
		Description: "claude-status is a CLI application",
	}
...
func aiWriteCompactCommand(b *strings.Builder, cmd *cobra.Command, prefix string) {
	...
	_, _ = fmt.Fprintf(b, "- `claude-status %s` - %s", name, cmd.Short)
